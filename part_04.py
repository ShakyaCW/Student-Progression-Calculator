...
def vertical_histogram():        #Vertical histogram
    ...
    for i in range (progress_count) :
        outcome_list[0].append ("*")    #first list is for progress count
    for i in range (trailer_count) :
        outcome_list[1].append ("*")    #second list is for trailer count
    for i in range (retriever_count) :
        outcome_list[2].append ("*")    #third list is for retriever count
    for i in range (exclude_count) :
        outcome_list[3].append ("*")    #forth list is for exclude count
...
def progression():    #print the elements of lists
    ...
    for m in progress_list:
        print (m)
        file = open("result.txt", "a")    #append the entered marks to the text file called result.txt
        file.write(m+"\n")
        file.close()
